...
searchB([F|_], M, NF, NF, NC) :- square(F, NC, M).
searchB([_|R], M, T, NF, NC) :- TF is T+1, searchB(R, M, TF, NF, NC).
...
game_over(_, B) :- blank_mark(E), \+ searchB(B, E,_, _, _). 

make_move(P, B) :- player(P, Type), make_move2(Type, P, B, B2), retract( board(_) ), asserta( board(B2) ). 

make_move2(humano, P, B, B2) :- 
                                player_mark(P, M),
                                findall((PF,PC), valido(B, M, PF, PC), L),
                                write(L),
                                nl, 
                                nl, 
                                write('El Jugador '), 
...
getPos(ul, F, C, NF, NC) :- NF is F-1, NC is C-1, F =\=1, C=\=1.
getPos(uc, F, C, NF, C) :-  NF is F-1, F=\=1.
getPos(ur, F, C, NF, NC) :- NF is F-1, NC is C+1, F=\=1, C=\=8.
getPos(cl, F, C, F, NC) :-  NC is C-1, C=\=1.
getPos(cr, F, C, F, NC) :- NC is C+1, C=\=8.
getPos(dl, F, C, NF, NC) :- NF is F+1, NC is C-1, F=\=8, C=\=1.
getPos(dc, F, C, NF, C) :- NF is F+1, F=\=8.
getPos(dr, F, C, NF, NC) :- NF is F+1, NC is C+1, F=\=8, C=\=8.
